// src/ingest/scoringRubricIngest.js
import fs from "fs";
...
function generateFakeEmbeddings(chunks, dim = 1536) {
  return chunks.map(() => Array.from({ length: dim }, () => Math.random()));
}

async function ingest() {
  try {
    const infile = path.resolve("src/data/scoring_rubric.txt");
    if (!fs.existsSync(infile)) {
      console.error("❌ ERROR: src/data/scoring_rubric.txt not found.");
      process.exit(1);
    }
    ...
    const ids = chunks.map((_, i) => `rubric-${Date.now()}-${i}`);

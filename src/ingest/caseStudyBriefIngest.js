// src/ingest/caseStudyBriefIngest.js
import fs from "fs";
...
async function ingest() {
  try {
    const infile = path.resolve("src/data/case_study_brief.txt");
    if (!fs.existsSync(infile)) {
      console.error("❌ ERROR: src/data/case_study_brief.txt not found.");
      process.exit(1);
    }
    ...
    const colName = "case_study_briefs";
    let collection;
    try {
    ...
    const ids = chunks.map((_, i) => `casebrief-${Date.now()}-${i}`);
